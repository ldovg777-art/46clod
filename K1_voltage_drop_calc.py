...
# Потребление трансмиттера Ekor (типовое):
#   - Электроника трансмиттера:  ~80 мА
#   - RS-485 приёмопередатчик:   ~15 мА
#   - 4-20 мА выход QS1 (макс): ~20 мА
#   - 4-20 мА выход QS2 (макс): ~20 мА
#   - Запас/пульсации:           ~15 мА
#   ИТОГО:                       ~150 мА (макс)
I_transmitter_scenarios = {
    'Минимальный (холостой ход, выходы 4мА)': 0.060,   # 60 мА
    'Номинальный (выходы ~12мА)':            0.120,     # 120 мА
    'Максимальный (выходы 20мА, макс нагр)': 0.150,    # 150 мА
    'Аварийный (пусковой бросок)':           0.200,     # 200 мА
}
...
print(f"""
  НАИХУДШИЙ СЛУЧАЙ (150 мА на трансмиттер, 65°C):
  ─────────────────────────────────────────────
...
V_25 = V_source - dU_25
print(f"     → V на трансмиттере (65°C, 150мА) = {V_25:.2f} В  ✅")

print(f"""
  2. Или ПОВЫСИТЬ напряжение источника до 27В (если блок питания позволяет):
     → V на трансмиттере (65°C, 150мА) = {V_worst + 3:.2f} В  ✅
...
print(f"     → V на трансмиттере (65°C, 150мА) = {V_100:.2f} В  ✅")
